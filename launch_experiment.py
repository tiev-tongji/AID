...
@click.option('--use_relabel', type=click.Choice(['true', 'false'], case_sensitive=False), default='false')
@click.option('--offline_data_quality', type=click.Choice(['mix', 'low', 'mid', 'expert'], case_sensitive=False), default='mix') # [mix, low, mid, expert]
def main(config, mujoco_version, gpu, seed, exp_name=None, pretrain=None, algo_type=None, train_z0_policy = None, use_hvar = None, z_strategy = None, r_thres=None, use_relabel=None, offline_data_quality=None):
    ''' launch experiment '''
    ...
    if not (use_relabel == None):
        variant['algo_params']['use_relabel'] = use_relabel.lower() == 'true'
    if not (offline_data_quality == None):
        variant['algo_params']['offline_data_quality'] = offline_data_quality

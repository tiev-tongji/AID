...
def plot_tensorboard_data(all_seed_data_dict, tags, plot_type='mean_std', save_path=None, names=None, tag_names=None, num_train_steps_per_itr=2000):
    ...
    # 每个 base_dir 对应一个颜色
    color_palette = sns.color_palette("tab10", len(all_seed_data_dict))  # 可选 "Set2" "tab10", "hsv", 或其他高对比度调色板
    ...
    for i, tag in enumerate(tags):
        for idx, (base_dir, all_seed_data) in enumerate(all_seed_data_dict.items()):
            ...
            # 使用Seaborn绘制带误差条的曲线图
            sns.lineplot(x=np.arange(len(smoothed_mean)) * (num_train_steps_per_itr / 1000), y=smoothed_mean, ax=axes[i], label=names[idx], color=color_palette[idx])

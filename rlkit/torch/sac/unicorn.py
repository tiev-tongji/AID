...
class UNICORNSoftActorCritic(OfflineMetaRLAlgorithm):
    def __init__(
            self,
            env,
            train_tasks,
        ...
        self.croo_lambda                    = kwargs['croo_lambda']
        self.unicorn_focal_weight           = kwargs['unicorn_focal_weight']
        self.loss                           = {}
        ...
        self.qf1, self.qf2, self.vf, self.c, self.context_decoder= nets[1:]
    ...
    @property
    def networks(self):
        return self.agent.networks + [self.qf1, self.qf2, self.vf, self.target_vf, self.c, self.context_decoder]
    ...
    def _take_step(self, indices, context):
        ...
        with torch.autograd.set_detect_anomaly(True):
            recon_loss = self.Recon_loss(context, task_z=self.agent.z)
            focal_z_loss = self.FOCAL_z_loss(indices=indices, task_z=task_z, task_z_vars=task_z_vars, b=obs.shape[1])
            unicorn_loss = recon_loss + self.unicorn_focal_weight * focal_z_loss
            unicorn_loss.backward()
        ...
        if self.eval_statistics is None:
            ...
            self.eval_statistics['UNICORN Loss'] = np.mean(ptu.get_numpy(unicorn_loss))
